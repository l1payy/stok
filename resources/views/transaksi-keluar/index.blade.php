                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($transaksi as $index => $item)
                        <tr class="hover:bg-gray-50 transition duration-150">
                            ...
                            <td class="px-6 py-4 text-sm font-bold text-center text-warning">- {{ $item->jumlah }}</td>
                            <td class="px-6 py-4 text-center">
                                @if(auth()->user()->role === 'admin')
                                    <form action="{{ route('transaksi-keluar.destroy', $item) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus catatan ini? Stok akan dikembalikan.')">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-10 text-center text-gray-400 italic">Belum ada riwayat stok keluar.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
